...
public class ConsoleView implements View {
    ...
    private final RuhasApp ruhasApp;
    private Scanner scanner;

    public ConsoleView(RuhasApp ruhasApp) {
        this.ruhasApp = ruhasApp;
    ...
    public void showRegistry() {
        ...
        List<AbstractAnimal> animals = ruhasApp.getAnimals();
    ...
    public boolean showAddAnimalDialog() {
        ...
        while (true) {
            ...
            try (counter){
                ...
                counter.add();
                return ruhasApp.createAnimal(parsedData.name, parsedData.birthDate, parsedData.animalGenius);
    ...
    public void showDetailInfoAnimalDialog() {
        String infoMessage = String.format("Введите номер животного (1 - %d)", ruhasApp.getAnimals().size());
        System.out.println(infoMessage);
        while (true) {
            try {
                scanner = new Scanner(System.in);
                int animalNumber = scanner.nextInt();
                AbstractAnimal animal = ruhasApp.getAnimals().get(animalNumber - 1);
    ...
    public int showRemoveAnimalDialog() {
        ...
        while (true) {
            try {
                ...
                return ruhasApp.removeAnimal(animal);
            } catch (IndexOutOfBoundsException e) {
                System.out.println("Запись с таким номером отсутствует");
            }
        }
